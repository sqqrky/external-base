...
#ifndef JM_hash_strHPP
#define JM_hash_strHPP
...
#define hash_char(str) ::jm::xor_string([]() { return str; }, std::integral_constant<std::size_t, sizeof(str) / sizeof(*str)>{}, std::make_index_sequence<::jm::detail::_buffer_size<sizeof(str)>()>{})
#define hash_str(str) hash_char(str).crypt_get()

#ifdef _MSC_VER
#define hash_strFORCEINLINE __forceinline
#else
#define hash_strFORCEINLINE __attribute__((always_inline)) inline
#endif
...
namespace jm {
	...
	namespace detail {
		...
		hash_strFORCEINLINE constexpr std::size_t _buffer_size()
		{
			return ((Size / 16) + (Size % 16 != 0)) * 2;
		}

		template<std::uint32_t Seed>
		hash_strFORCEINLINE constexpr std::uint32_t key4() noexcept
		{
		...
		hash_strFORCEINLINE constexpr std::uint64_t key8()
		{
		...
		hash_strFORCEINLINE constexpr std::uint64_t
		...
		hash_strFORCEINLINE std::uint64_t load_from_reg(std::uint64_t value) noexcept
		{
...
#ifndef JM_hash_strDISABLE_AVX_INTRINSICS
	...
	public:
		...
		template<class L>
		hash_strFORCEINLINE xor_string(L l, std::integral_constant<std::size_t, Size>, std::index_sequence<Indices...>) noexcept
			: _storage{ ::jm::detail::load_from_reg((std::integral_constant<std::uint64_t, detail::load_xored_str8<Size>(Keys, Indices, l())>::value))... }
		{
		}

		hash_strFORCEINLINE constexpr size_type size() const noexcept
		{
			return Size - 1;
		}

		hash_strFORCEINLINE void crypt() noexcept
		{
...
#endif
#elif !defined(JM_hash_strDISABLE_AVX_INTRINSICS)
		...
		hash_strFORCEINLINE const_pointer get() const noexcept
		{
			return reinterpret_cast<const_pointer>(_storage);
		}

		hash_strFORCEINLINE pointer get() noexcept
		{
			return reinterpret_cast<pointer>(_storage);
		}

		hash_strFORCEINLINE pointer crypt_get() noexcept
		{
